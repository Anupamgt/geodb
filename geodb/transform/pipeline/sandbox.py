...
def check_undefined_calls(code: str) -> list:
    """
    Detect calls to plain-name functions that are not defined or imported
    in the generated code. Returns list of undefined name strings.

    Ignores Python builtins and variables injected by the sandbox wrapper
    ...
    # Seed with builtins and sandbox-injected names
...
def _collect_names(target, defined: set):
    """Recursively collect assigned variable names from an AST target node."""
...
def _check_import(module_name: str, violations: list):
    ...
    # Allow if exact match or is a sub-module of an allowed package
